...
import type { AggregatedBucket, MetricField } from "../../../data/types/aggregation";
...
function TimelineChart({ buckets, metricField }: TimelineChartProps) {
  const { points, maxValue } = useMemo(() => {
    ...
    const points = buckets.map((bucket, index) => {
      const x = PADDING_LEFT + (buckets.length === 1 ? innerWidth / 2 : (index / (buckets.length - 1)) * innerWidth);
  ...
  return (
    <div className={styles.container}>
      <div className={styles.header}>
        <h2 className={styles.title}>{metricFieldLabel(metricField)} over time</h2>
      ...
      {buckets.length === 0 ? (
        ...
        <div className={styles.svgContainer}>
          ...
          >
            <rect x={0} y={0} width={WIDTH} height={HEIGHT} fill="var(--color-surface, #0b1120)" rx={12} />

            {yTicks.map((tick, idx) => (
              <g key={idx}>
                <line x1={PADDING_LEFT} x2={WIDTH - PADDING_RIGHT} y1={tick.y} y2={tick.y} stroke="rgba(148, 163, 184, 0.25)" strokeWidth={1} />
                <text x={PADDING_LEFT - 8} y={tick.y + 4} textAnchor="end" fontSize={11} fill="#94a3b8">
                  {tick.label}
                </text>
              </g>
            ))}

            <line x1={PADDING_LEFT} x2={WIDTH - PADDING_RIGHT} y1={HEIGHT - PADDING_BOTTOM} y2={HEIGHT - PADDING_BOTTOM} stroke="#64748b" strokeWidth={1.2} />

            {xLabels.map((tick, idx) => (
              <text key={idx} x={tick.x} y={HEIGHT - PADDING_BOTTOM + 16} textAnchor="middle" fontSize={10} fill="#94a3b8">
                {tick.label}
              </text>
            ))}
            ...
            {points.map((p, idx) => (
              <circle key={idx} cx={p.x} cy={p.y} r={3} fill="#38bdf8" stroke="#0f172a" strokeWidth={1} />
            ))}
          </svg>
        </div>
      )}
